...
Training script for the multimodal early-fusion Vision Mamba baseline.
...
Expected model file:
    models/baselines/vision_mamba_baseline.py

Expected config:
    configs/vision_mamba_base.yaml
"""
...
class BaselineSegmentationLoss(nn.Module):
    ...
    auxiliary-branch losses so Vision Mamba is not forced to produce outputs
    it does not have.
    """
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the multimodal Vision Mamba baseline."
    )
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("train_vision_mamba")
...
def main() -> None:
    ...
    default_output_dir = cfg.get("output", {}).get(
        "checkpoint_dir",
        "outputs/checkpoints/Vision Mamba_200ep_seed2026",
    )
