"""Save Vision Mamba predictions for later multi-model paper panels."""
...
from models.baselines.vision_mamba_baseline import MultimodalVisionMamba
from utils.config import load_config


def parse_args():
    parser = argparse.ArgumentParser(
        description="Export predictions from the multimodal Vision Mamba baseline."
    )
    parser.add_argument(
        "--config",
        default="configs/vision_mamba_base.yaml",
    )
    parser.add_argument(
        "--checkpoint",
        required=True,
    ...
    parser.add_argument(
        "--save_dir",
        default="outputs/predictions/VisionMamba_200ep_seed2026",
    )
    parser.add_argument(
        "--threshold",
        type=float,
...
def build_model(cfg: dict) -> MultimodalVisionMamba:
    baseline_cfg = cfg.get("baseline", {})

    return MultimodalVisionMamba(
        ...
        expansion=int(
            baseline_cfg.get("expansion", 2)
        ),
        kernel_size=int(
            baseline_cfg.get("kernel_size", 7)
        ),
        dropout=float(
...
def extract_ids(batch: dict, batch_size: int, start: int) -> List[str]:
    result = []
...
def optical_to_rgb(
    ...
    return np.stack(
        [
            percentile_stretch(rgb[..., channel])
...
def save_overlay(
    ...
    overlay[mask, 0] = (
        (1.0 - alpha) * overlay[mask, 0] + alpha
    )
    overlay[mask, 1] = (
...
def save_preview(
    ...
    panels = [
        ...
        (prediction, "Vision Mamba Prediction", "gray"),
...
def main() -> None:
    ...
    if len(args.rgb_indices) != 3:
        raise ValueError(
            "--rgb_indices must contain exactly three indices."
        )

    optical_channels = int(
        cfg["dataset"]["optical_channels"]
    )
    if any(
        index < 0 or index >= optical_channels
        for index in args.rgb_indices
    ):
        raise ValueError(
            f"--rgb_indices must lie between 0 and "
            f"{optical_channels - 1}."
        )

    requested = cfg["training"].get(
        "device",
        "cuda",
    )
    device = torch.device(
    ...
    for batch in tqdm(
        loader,
        desc="Saving Vision Mamba predictions",
    ):
        sar = batch["sar"].to(
            device,
            non_blocking=True,
        )
        optical = batch["optical"].to(
            device,
            non_blocking=True,
        )
        mask = batch["mask"].to(
        ...
        for index, sample_id in enumerate(sample_ids):
            ...
            np.save(
                directories["prob_npy"]
                / f"{sample_id}.npy",
                probability,
            )
            np.save(
                directories["binary_npy"]
                / f"{sample_id}.npy",
                prediction,
            )

            torch.save(
                probabilities[index, 0].cpu(),
                directories["prob_pt"]
                / f"{sample_id}.pt",
            )
            torch.save(
                binaries[index, 0].cpu(),
                directories["binary_pt"]
                / f"{sample_id}.pt",
            )

            save_gray(
                probability,
                directories["prob_png"]
                / f"{sample_id}.png",
            )
            save_gray(
                prediction,
                directories["binary_png"]
                / f"{sample_id}.png",
            )
            save_gray(
                target,
    ...
    write_csv(
        rows,
        root / "per_image_metrics.csv",
    )

    print("\n" + "=" * 68)
    print(
        "VISION MAMBA PREDICTION EXPORT COMPLETED"
    )
    print("=" * 68)
    ...
    print(
        f"Probabilities : "
        f"{directories['prob_npy']}"
    )
    print(
        f"Binary masks  : "
        f"{directories['binary_npy']}"
    )
    print(
        f"Overlays      : "
        f"{directories['overlay']}"
    )
    print(
        f"Preview panels: "
        f"{directories['preview']}"
    )
    print(
        f"Metrics CSV   : "
        f"{root / 'per_image_metrics.csv'}"
    )
    print("=" * 68)
