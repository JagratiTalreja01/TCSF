...
Training script for the multimodal early-fusion DeepLabV3+ baseline.
...
Expected model file:
    models/baselines/deeplabv3plus_baseline.py

Expected config:
    configs/deeplabv3plus_base.yaml
"""
...
from models.baselines.deeplabv3plus_baseline import MultimodalDeepLabV3Plus
...
class BaselineSegmentationLoss(nn.Module):
    ...
    auxiliary-branch losses so the U-Net is not forced to produce outputs
    it does not have.
    """
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the multimodal DeepLabV3+ baseline."
    )

    parser.add_argument(
        "--config",
        type=str,
        default="configs/deeplabv3plus_base.yaml",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("train_deeplabv3plus")
...
def build_model(cfg: dict) -> MultimodalDeepLabV3Plus:
    baseline_cfg = cfg.get("baseline", {})

    return MultimodalDeepLabV3Plus(
        sar_channels=cfg["dataset"]["sar_channels"],
        optical_channels=cfg["dataset"]["optical_channels"],
        num_classes=cfg["dataset"]["num_classes"],
        pretrained_backbone=bool(
            baseline_cfg.get("pretrained_backbone", False)
        ),
        aspp_channels=int(
            baseline_cfg.get("aspp_channels", 256)
        ),
...
def main() -> None:
    ...
    default_output_dir = cfg.get("output", {}).get(
        "checkpoint_dir",
        "outputs/checkpoints/DeepLabV3Plus_200ep_seed2026",
    )
