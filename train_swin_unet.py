...
Training script for the multimodal early-fusion Swin-UNet baseline.
...
Expected model file:
    models/baselines/swin_unet_baseline.py

Expected config:
    configs/swin_unet_base.yaml
"""
...
class BaselineSegmentationLoss(nn.Module):
    ...
    auxiliary-branch losses so Swin-UNet is not forced to produce outputs
    it does not have.
    """
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the multimodal Swin-UNet baseline."
    )

    parser.add_argument(
        "--config",
        type=str,
        default="configs/swin_unet_base.yaml",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def setup_logger(log_path: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("train_swin_unet")
...
def build_model(cfg: dict) -> MultimodalSwinUNet:
...
def main() -> None:
    ...
    default_output_dir = cfg.get("output", {}).get(
        "checkpoint_dir",
        "outputs/checkpoints/Swin-UNet_200ep_seed2026",
    )
